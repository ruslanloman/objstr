...
/// Get the version string from `rawobjstr --version`.
pub fn cli_version() -> String {
    run_cli(&["--version"]).trim().to_string()
}

/// Assert the version string has not changed since the start of the test run.
/// Call this on error or at the end of a test run to detect binary replacement.
pub fn assert_version_unchanged(expected: &str) {
    let current = cli_version();
    assert_eq!(
        current, expected,
        "rawobjstr binary version changed during test run!\n  start: {expected}\n  now:   {current}\n\
         The binary may have been replaced or rebuilt while tests were running."
    );
}
...
// ---------------------------------------------------------------------------
// Low-level file I/O helpers (corruption / crash tests)
// ---------------------------------------------------------------------------

/// Flip all bits in a single byte at `byte_offset` within the file at `path`.
pub fn flip_byte(path: &std::path::Path, byte_offset: u64) {
...
/// Overwrite a range with zeros.
pub fn zero_range(path: &std::path::Path, offset: u64, len: usize) {
    use std::fs::OpenOptions;
...
/// Read `len` bytes from offset in a file.
pub fn read_bytes_at(path: &std::path::Path, offset: u64, len: usize) -> Vec<u8> {
    use std::fs::File;
...
/// Write arbitrary bytes at a given offset.
pub fn write_bytes_at(path: &std::path::Path, offset: u64, data: &[u8]) {
    use std::fs::OpenOptions;
    use std::io::{Seek, SeekFrom, Write};
    let mut f = OpenOptions::new().read(true).write(true).open(path).unwrap();
    f.seek(SeekFrom::Start(offset)).unwrap();
...
pub fn truncate_file(path: &std::path::Path, size: u64) {
    use std::fs::OpenOptions;
