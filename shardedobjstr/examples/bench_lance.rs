...
//! with ShardedObjectStore directly (no Python FFI overhead).
//!
...
//!   img+directio -- ShardedObjectStore (1 raw shard, rf=1), 16 GB image, O_DIRECT
//!   img+buffered -- ShardedObjectStore (1 raw shard, rf=1), 16 GB image, buffered
//!   raw+directio -- ShardedObjectStore (1 raw shard, rf=1), /dev/sdb, O_DIRECT
//!   raw+buffered -- ShardedObjectStore (1 raw shard, rf=1), /dev/sdb, buffered
//!
//! Usage:
...
// -- Provider: plugs ShardedObjectStore into LanceDB ----------------------

struct ShardedStoreProvider {
    store: Arc<ShardedObjectStore>,
}

impl fmt::Debug for ShardedStoreProvider {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ShardedStoreProvider").finish()
    }
}

#[async_trait]
impl ObjectStoreProvider for ShardedStoreProvider {
    async fn new_store(
        &self,
        base_path: Url,
...
struct ShardedBackend {
    ...
    size: Option<u64>,
    direct_io: bool,
    raw_store: Option<Arc<RawObjectStore>>,
    cluster: Option<Arc<ShardedObjectStore>>,
}
...
impl Backend for ShardedBackend {
    ...
    fn connect(&mut self, _rt: &tokio::runtime::Runtime) -> (String, Option<Arc<lancedb::Session>>) {
        let dev_size = if let Some(sz) = self.size {
            sz
        } else {
        ...
        let stores: Vec<Arc<dyn ObjectStore>> = vec![raw.clone() as _];
        let cluster = Arc::new(ShardedObjectStore::new(stores, 1));

        self.raw_store = Some(raw);
        self.cluster = Some(Arc::clone(&cluster));

        let provider = Arc::new(ShardedStoreProvider { store: cluster });
        let registry = ObjectStoreRegistry::default();
        registry.insert("sharded", provider);
        let session = Arc::new(lancedb::Session::new(128, 128, Arc::new(registry)));

        ("sharded:///".to_string(), Some(session))
    }

    fn cleanup(&mut self) {
        if let Some(ref raw) = self.raw_store {
            raw.flush_index().unwrap();
        }
        self.cluster = None;
        self.raw_store = None;
...
fn run_benchmark(
    ...
    // Explicitly drop the database connection and session so all Arc references
    // are released before cleanup() tries to format the same device.
    drop(db);
    drop(session);
...
fn main() {
    ...
    println!("=== Lance Sharded Benchmark (Rust) ===");
    println!(
        "ShardedObjectStore with 1 raw shard, rf=1");
    println!(
    ...
    for &name in &selected {
        ...
        let mut backend: Box<dyn Backend> = match name {
            ...
            "img+directio" => Box::new(ShardedBackend::new(
                "/tmp/lance_sharded_dio.img",
                Some(img_size),
                true,
                "img+directio",
            )),
            "img+buffered" => Box::new(ShardedBackend::new(
                "/tmp/lance_sharded_buf.img",
                Some(img_size),
                false,
                "img+buffered",
            )),
            "raw+directio" => Box::new(ShardedBackend::new(
                &device,
                None,
                true,
                "raw+directio",
            )),
            "raw+buffered" => Box::new(ShardedBackend::new(
                &device,
                None,
                false,
                "raw+buffered",
        ...
        // Arc references (e.g. lancedb background tasks).
